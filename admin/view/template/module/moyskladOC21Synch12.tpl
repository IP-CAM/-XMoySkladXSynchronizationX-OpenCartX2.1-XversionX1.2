            ...
            <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-category" class="form-horizontal">
               <table class="form">
                    <tr>
                      ...
                      <td><input name="moyskladOC21Synch12_username" type="text" value="<?php echo $moyskladOC21Synch12_username; ?>" /></td>
                    </tr>
                    <tr>
                      <td><?php echo $entry_password; ?></td>
                      <td><input name="moyskladOC21Synch12_password" type="password" value="<?php echo $moyskladOC21Synch12_password; ?>" /></td>
                    </tr>
                    <tr>
                    <td><?php echo $entry_order_status_to_exchange; ?></td>
                    <td>
                      <select name="moyskladOC21Synch12_order_status_to_exchange">
                        <option value="0" <?php echo ($moyskladOC21Synch12_order_status_to_exchange == 0)? 'selected' : '' ;?>><?php echo $entry_order_status_to_exchange_not; ?></option>
                        <?php foreach ($order_statuses as $order_status) { ?>
                        <option value="<?php echo $order_status['order_status_id'];?>" <?php echo ($moyskladOC21Synch12_order_status_to_exchange == $order_status['order_status_id'])? 'selected' : '' ;?>><?php echo $order_status['name']; ?></option>
                        <?php } ?>
                      </select>
                    </td>
                    </tr>
